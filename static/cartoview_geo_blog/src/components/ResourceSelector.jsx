...
export default class ResourceViewer extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      showPagination: true,
      pageCount: 0,
      selected: this.props.instance ? this.props.instance.app : {}
    }
  }
  loadResources(off) {
      ...
      <div>
        <div className="row">
          ...
          <div className="col-xs-12 col-sm-4 col-md-4 col-lg-4">
            { (this.state.selected.id || (this.props.instance ? this.props.instance.app : false)) && <button className="btn btn-primary pull-right" onClick={()=>this.props.onComplete(this.state.selected)}>Next</button>}
          </div>

        </div>
        {this.state.resources.map((app) => {
          return <div onClick={() => this.setState({selected: app})} key={app.id} className={(this.state.selected && this.state.selected.id == app.id) || (this.props.instance ?  this.props.instance && this.props.instance.app.id == app.id : false )
            ...
            app.thumbnail_url, app.map ? app.map.thumbnail_url : "/static/" + app.app.name + "/logo.png","/static/app_manager/img/no-image.jpg"
          ]}/></div>
